...
public class RoomPanel extends JPanel implements MouseListener
{
    ...
    Color midnight = new Color(0, 0, 139);
    Color navy = new Color(0, 0, 128);
    Color royal = new Color(65, 105, 225);
    Color dodger = new Color(30, 144, 255);
    Color cornflower = new Color(100, 149, 237);
    Color sky = new Color(135, 206, 235);
    Color steel = new Color(70, 130, 180);
    Color turqouise = new Color(0, 199, 140);
    Color light = new Color(173, 216, 230);
      ...
      for (int r = 0 ; r < grid.length ; r++)
      {
         ...
         for (int c = 0 ; c < grid[r].length ; c++)
         {
            ...
            if (people[r][c].isInfected())
            {
               ...
               if (people[r][c].getImmuneRate() >= 70)
               {
                  grid[r][c].setBackground(light);
               }
               else if (people[r][c].getImmuneRate() >= 60)
               {
                  grid[r][c].setBackground(turqouise);
               }
               else if (people[r][c].getImmuneRate() >= 50)
               {
                  grid[r][c].setBackground(steel);
               }
               else if (people[r][c].getImmuneRate() >= 40)
               {
                  grid[r][c].setBackground(sky);
               }
               else if (people[r][c].getImmuneRate() >= 30)
               {
                  grid[r][c].setBackground(cornflower);
               }
               else if (people[r][c].getImmuneRate() >= 20)
               {
                  grid[r][c].setBackground(dodger);
               }
            }
    ...
    private int countNumInfected()   //Count number of infected/red tiles
    {
        ...
        for (int r = 0 ; r < people.length ; r++)
      {
        ...
        return num;  //Temporary statement to allow template compile
    }
    //Minor methods END
    ...
    private class RandomInfectListener implements ActionListener
    {
        ...
        public void actionPerformed(ActionEvent e)
        {
            int randomRow = (int) (Math.random() * people.length) + 1;
            int randomCol = (int) (Math.random() * numCols) + 1;
            
            infect(randomRow, randomCol);
            
            randomRow = (int) (Math.random() * people.length) + 1;
            randomCol = (int) (Math.random() * numCols) + 1;
         ...
         for (int r = 0 ; r < rows ; r++)
         {
            ...
            for (int c = 0 ; c < cols ; c++)
            {
               ...
               if (people[r][c].isInfected())
               {
                  ...
                  if (r > 0 ) 
                     if (1 <= people[r][c].compareTo(people[r - 1][c]))
                     {
                  ...
                  if (c > 0 )
                     if (1 <= people[r][c].compareTo(people[r][c - 1]))
                        temp[r][c - 1] = 1;  
